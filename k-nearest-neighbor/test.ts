...
import {Node, IKeys, NodeList} from './node';
import {expect} from 'chai';

describe('Service: k-nearest-neighbor', () =>  {
  it('should classify a flat', () => {
        let neighbors = 3;
        let nodes = new NodeList(neighbors);
        loadData(nodes);
        let input = getFlat();
        addUnknownNode(nodes, input);
        let result = nodes.determineUnknown();
        expect(result).to.have.deep.property(input.type, neighbors);
    });

     it('should classify apartment', () => {
        let neighbors = 3;
        let nodes = new NodeList(neighbors);
        loadData(nodes);
        let input = getApt();
    ...
    it('should classify the closest points with the greatest count', () => {
      /*
      ...
      */
        ...
        let nodes = new NodeList(neighbors);
        loadData(nodes);
        let input = {rooms: 8, area: 280, type: 'house'};
        addUnknownNode(nodes, input);
        let result = nodes.determineUnknown();
        let processed = isMultipleKey(result) ?  getHighest(result) : result;
        expect(processed).to.have.deep.property(input.type);
    });

  describe('Service: Node', () => {
    it('should new up a new Node', () => {
        let obj:IKeys = {rooms: 1, area: 350, type: 'apartment'};
        let a = new Node(obj);
        expect(a).to.have.deep.property('rooms', 1);
    });
    it('should measure distances between 2 points', () => {
        let obj:IKeys = {rooms: 1, area: 350, type: 'apartment'};
        let a = new Node(obj);
        expect(a).to.have.deep.property('rooms', 1);
    });

    it('should find min and max by traversing all nodes', () => {
      let neighbors = 3;
        let nodes = new NodeList(neighbors);
        loadData(nodes);
        nodes.calculateRanges();
        expect(nodes.rooms).to.deep.equal({ min: 1, max: 10 });
        expect(nodes.areas).to.deep.equal({max: 1700, min: 250});
    });

    it('should normalize data if asked nicely', () => {
        let result = normalizeFn(8, {min: 1, max: 10}, 3);
  ...
  describe('Service: utilities', () => {
    it('should new up a new Node', () => {
        let obj:any = {rooms: 349, area: 350};
        let res = getHighest(obj);
        expect(res).to.have.deep.property('area', 350);
    });
  ...
  function addUnknownNode(nodes, obj) {
      nodes.add( new Node({rooms: obj.rooms, area: obj.area, type: false}) );
  }
